    ...
    <header :class="[bemm('hero'), 'fade-up']">
      <div :class="bemm('hero-copy')">
        ...
        <div :class="bemm('hero-actions')">
          ...
          <a href="#contribution-form">Learn how it works <Icon name="arrow-right" size="small" /></a>
        </div>
      </div>
      ...
      <section :class="[bemm('workflow'), 'reveal']" aria-label="Contribution workflow">
        ...
        <ol>
          <li v-for="(item, index) in workflow" :key="item.title">
            ...
            <h2>{{ item.title }}</h2>
            <p>{{ item.copy }}</p>
            <Icon v-if="index < workflow.length - 1" name="arrow-right" size="small" :class="bemm('step-arrow')" />
          </li>
        </ol>
      </section>
